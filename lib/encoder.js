...
Encoder.encode = function(route, msg){
	...
	if(!!protos){
		try{
			...
			throw new Error('failed encoding msg ' + ex.toString() + ':' + JSON.stringify([route,protos,msg]));
		}
		if(offset > 0){
	...
	for(var name in msg){
		if(!!protos[name]){
			...
			try{
				...
				throw new Error('failed encoding prop '+ name + ': ' + ex.toString());
			}
...
function encodeProp(value, type, offset, buffer, protos){
	...
	if (value===null || value===undefined){
		throw new Error('null prop to encode');
	}
	switch(type){
		case 'uint32':
